...
- **只做分享，拒绝任何可能的商业化行为。**
- **已做「带上下文重译」**：全部 21 个剧情文件按「对话回合」而非单句重新审校，并统一了跨文件术语
  （详见 `knowledge-base/workflow/重译手册.md`）。但 AI 翻译**仍可能有错**，请当**尝鲜**用，
  之后**务必换回都市零协会的汉化**。
- 像 **以实玛利·Le Rouge 精品店** 这类术语难以译成中文，**保留拉丁原文**；本批新拟的译名在**首次出现处标注了英文原文**（如「西西弗之刑（Sisyphe Penalty）」）。
